...
{
	u8 i;	
	input_rev=ParamGetFromName("INPUT_REV");
	s32 t=*input_rev;
	for(i=0;i<8;i++)
	{
		pwmValues[i]=0;
...
u16 getCap(u8 i)
{
	switch(i)
	{
		...
		case 6:return TIM_GetCapture3(TIM4);
		case 7:return TIM_GetCapture4(TIM4);
	}
}
...
void setPolarity(u8 i,u16 p)
{
	switch(i)
	{
		...
		case 6:TIM_OC3PolarityConfig(TIM4,p);return;
		case 7:TIM_OC4PolarityConfig(TIM4,p);return;
	}
}
